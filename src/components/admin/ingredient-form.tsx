...
export function IngredientForm() {
  ...
  async function createIngredient(value: string) {
    setError("");
    setLoading(true);
    try {
  ...
  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (name.trim()) createIngredient(name);
  }

  return (
    <div className="space-y-4 rounded-2xl bg-white p-6 shadow-card">
      <h2 className="font-semibold text-gray-900">Add a food</h2>
      ...
      <div className="flex items-center gap-3 text-xs text-gray-400">
        <div className="h-px flex-1 bg-gray-200" /> or type it manually <div className="h-px flex-1 bg-gray-200" />
      </div>
